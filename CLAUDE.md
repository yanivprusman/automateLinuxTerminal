...
1. **`automateLinuxTerminalMenu`** — triggered by right-clicking the clock/status area. Shows the active Claude session ID (first 8 chars) and Claude's working directory. Items are hover-highlighted.
   ...
   - Top to bottom: **topic → pin topic → sessions → timer → `?`**. The topic owns the first row under the border because it is what the menu is opened for; the `?` (name and version, unfolding in place) owns the last, because it is the least-wanted thing here and holding the top row it pushed the topic one further from the pointer on every open. Last also makes its info line free: it grows the menu downwards and moves no row above it. Nothing may assume a fixed offset for either — `helpRowOff` travels in `ContextMenuState` with `topicRowOff`/`sessionsRowOff`/`stopwatchRowOff`, since the `?` now moves with every session that appears above it.
